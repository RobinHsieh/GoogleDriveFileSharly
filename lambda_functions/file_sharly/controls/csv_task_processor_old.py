...
class CSVTaskProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            course_list (list[dict]): The list of dictionaries containing course informations.
        ...
        self.file_id_list = course_list
        self.file_id_and_course_name_map = {}  # cache the file_id and course_name
    ...
    @staticmethod
    def _convert_to_shareable_course_name_and_if_successed_map(
        shareable_course_name_list: list[str],
    ) -> dict[str, bool]:
        shareable_course_name_and_if_successed_map = {}
        for course_name in shareable_course_name_list:
            shareable_course_name_and_if_successed_map[course_name] = False
        return shareable_course_name_and_if_successed_map
    ...
    def _process_single_user_row(
        ...
        """
        shareable_file_id_list = [] # user
        shareable_course_name_list = [] # user
        offset = -1 # user
        view_limit_near_course_list = [] # user

        column_date_start_from = 5
        future_list = []
        reason_response = ReviewResult(
            reason="空白。",
            total_score=0,
        ...
        if not self._if_user_has_enrolled(row):
            ...
            shareable_course_name_and_if_successed_map = (
                CSVTaskProcessor._convert_to_shareable_course_name_and_if_successed_map(
                    shareable_course_name_list
                )
            )
            return ShareLog(
                row=row,
                email=email_address,
                shareable_course_name_and_if_successed_map=shareable_course_name_and_if_successed_map,
                offset=offset,
            )
        ...
        with ThreadPoolExecutor(max_workers=12) as executor:
            for course_index, date_column_index in enumerate(
                ...
                course_name = self.file_id_list[course_index].get("SK")
                ...
                """
                Save the file_id and course_name in the map.
                """
                self.file_id_and_course_name_map[
                    self.file_id_list[course_index]["FileID"]
                ] = course_name
            ...
            for task in as_completed(future_list):
                ...
                if_view_limit_near = cell_state["if_view_limit_near"]
                if_view_limit_reached = cell_state["if_view_limit_reached"]

                """
                review the reason
                """
                if if_view_limit_near and reason_response.explanation == "尚未初始化。":
                    reason_response = (
                ...
                """
                user_state = (
                    self.property_and_state_manager.get_updated_user_state(
                        course_index, # cell
                        if_view_limit_near, # cell
                        reason_response, # user
                        if_view_limit_reached, # cell
                        self.file_id_list,
                        shareable_file_id_list, # user
                        shareable_course_name_list, # user
                        offset, # user
                ...
                shareable_file_id_list = user_state["shareable_file_id_list"]
                shareable_course_name_list = user_state["shareable_course_name_list"]
                offset = user_state["offset"]
                view_limit_near_course_list = user_state["view_limit_near_course_list"]
        ...
        if offset > 0:
            ...
            email_message = CSVTaskProcessor._generate_email_message(
                expire_date_utc8, view_limit_near_course_list, self.view_limit_int
            )
