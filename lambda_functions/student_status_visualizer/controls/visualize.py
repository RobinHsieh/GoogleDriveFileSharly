...
def status(sheet_client: GoogleSheetClient, target_date: str, red, green, blue, status: str):
    ...
    try:
        ...
        if target_date not in sheet_data_frame.columns:
            print(f"找不到日期欄位為 {target_date} 的課程")
        else:
            ...
            if absent_data_frame.empty:
                print(f"{target_date} 沒有{status}的記錄。")
            else:
    ...
    try:
        ...
        if target_date not in sheet_data_frame.columns:
            ...
            query_status = "臨時請假" if status == "請假" else status
            absent_mask = sheet_data_frame["申請類別"].isin([query_status]) & sheet_data_frame[target_date].isin(["V"])
            absent_data_frame = sheet_data_frame[absent_mask]

            if absent_data_frame.empty:
                print(f"{target_date} 沒有{query_status}的記錄。")
            else:
                print(f"找到 {len(absent_data_frame)} 筆 {target_date} 的{query_status}記錄。")
        ...
        print(f"找不到或讀取同步直播工作表失敗，跳過此步驟。錯誤原因: {e}")
